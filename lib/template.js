        ...
        else if ((nodes = getChildNodes(node)) && nodes.length > 0)
        {
            if (tagName === 'databox')
            {
                try
                {
                    ...
                    indexStack.push(scope[1], node.getAttribute('type') === 'model');

                    if (nodes.length > 1)
                    {
                        console.error('databox node can only include one child node!');
                    }

                    parseDataBox(array, nodes[0], space + '\t');
                }
                finally
                {
                    scopeStack.pop();
...
function parseBindings(array, bindings, space) {
    ...
    while (name = bindings[index++])
    {
        ...
        else
        {
            ...
            array.push(space, '"', name, '":  function ($pipe) { return ', value, ' }');
        }
    }
}
...
function parseDataBox(array, node, space) {
