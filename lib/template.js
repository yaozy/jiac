...
function parse(array, node, space) {
    ...
    switch (tagName)
    {
        ...
        case 'template':
            array.push(space, 'require("', node.getAttribute('src'), '").call(this, ', 
...
function parseDataBox(array, nodes, space) {
    ...
    while (last--)
    {
        scope[last] = space + '\tvar ' + scope[last] + ' = scope[' + last + '];\n';
    }

    array.push(',\n', space, 'function (template, data, scope) {\n',
        scope.length > 0 ? '\n' : '',
        scope.join(''),
        '\n',
        space, '\tfor (var ', index, ' = 0, length = data.length; ', index, ' < length; ', index, '++)\n',
        space, '\t{\n',
        space, '\t\t// 添加作用域解决循环变量绑定变化的问题\n',
        space, '\t\t(function (', index, ', ', item, ') {\n\n',
        '\n',
...
module.exports = function (text, file) {
    ...
    try
    {
        ...
        {
            array = ['module.exports = function ($this, $data, $model) {\n\n',
                'return (\n'];
